...
from agentflow.client import DevTaskLoader
...
async def server_setup() -> AsyncGenerator[Dict[str, Any], None]:
    """
    A pytest fixture to manage the lifecycle of the AgentFlowServer.
    ...
    """
    server = AgentFlowServer(host="127.0.0.1", port=8008, task_timeout_seconds=2.0)
    await server.start()

    # httpx.AsyncClient is used for direct HTTP requests to the server endpoints
    # within the tests, which is more reliable than spinning up a full AgentFlowClient
...
async def test_uri_and_semantic_correctness(server_setup: Dict[str, Any], sample_resources: NamedResources):
    ...
    """
    server: AgentFlowServer = server_setup["server"]
    ...
    client = AgentFlowClient(endpoint=endpoint)
...
async def test_full_lifecycle_async(
    ...
    server: AgentFlowServer = server_setup["server"]
    endpoint: str = server_setup["endpoint"]
    client = AgentFlowClient(endpoint=endpoint, poll_interval=0.1)
...
async def test_client_with_bad_endpoint():
    ...
    client = AgentFlowClient(endpoint="http://127.0.0.1:9999", timeout=0.5)
